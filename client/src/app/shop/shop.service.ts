...
export class ShopService {
  ...
  getProducts(useCache = true): Observable<Pagination<Product[]>> {
    ...
    ); // Can do {params: params} too unless variable name is different
  }
  ...
  getProduct(id: number) {
    const product = [...this.productCache.values()]
     ...
     if (Object.keys(product).length !== 0) return of(product); // of functions returns the product as observable
